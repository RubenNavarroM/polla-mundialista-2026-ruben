...
// Manual score correction: Spain vs Saudi Arabia ended 5-0 but API returned 4-0
function applyScoreOverrides(match: Match): Match {
  ...
  if (spainIsHome) {
    return { ...match, home_score: 5, away_score: 0 };
  }
  if (spainIsAway) {
    return { ...match, home_score: 0, away_score: 5 };
  }
  return match;
}
